...
async function addPeople(movie_id: string, names: string[], table: string, extra_data?: { key: string, value: string }[]) {
    ...
    for (const name of names) {
        logger.info(`Adding ${personType} "${name}" to movie ID ${movie_id}`);
        ...
        if (people.length === 0) {
            ...
            logger.info(`No existing ${personType} found with name "${name}", created new entry with ID ${personID}`);
        } else {
            logger.info(`Found existing ${personType} with name "${name}", using ID ${people[0].id}`);
            personID = people[0].id;
        }
        const tableName = `movie_${table}`;
        await db`INSERT INTO ${db(tableName)} (movie_id, person_id) VALUES (${movie_id}, ${personID})`;
        logger.info(`Linked ${personType} ID ${personID} to movie ID ${movie_id} in table ${tableName}`);
        if (extra_data) {
            logger.info(`Adding extra data for ${personType} "${name}" in movie ID ${movie_id}: ${JSON.stringify(extra_data)}`);
...
export async function addMovie(movie: MovieInput): Promise<string> {
    ...
    logger.info(`Added movie "${movie.title}" with ID ${newID} to the database`);

    if (movie.directors.length === 0) {
        logger.warn(`No directors provided for movie "${movie.title}" (ID ${newID})`);
    }
    else{
        await addPeople(newID, movie.directors, 'directors');
    }
    if (movie.writers.length === 0) {
        logger.warn(`No writers provided for movie "${movie.title}" (ID ${newID})`);
    }
    else{
    ...
    if (movie.poster_image_url) {
        ...
        try {
            ...
            logger.error( error, `Failed to add poster image from URL ${movie.poster_image_url} for movie ID ${newID}:`);
        }
    }
    else {
        logger.warn(`No poster image URL provided for movie "${movie.title}" (ID ${newID})`);
    }
    return newID;
}
...
export async function getMovieByIMDBID(imdb_id: string): Promise<Movie | null> {
    ...
    if (movies.length === 0) {
        logger.info(`No movie found in our database with IMDB ID ${imdb_id}`);
        return null;
    }
    logger.info(`Found movie in our database with IMDB ID ${imdb_id}: "${movies[0].title}" (ID ${movies[0].id})`);
    return movies[0];
}
...
export async function editMovie(movie: Movie): Promise<void> {
    ...
    logger.info(`Updated movie "${movie.title}" (ID ${movie.id}) in the database`);
}

export async function deleteMovie(movie_id: string): Promise<void> {
    await db`DELETE FROM movies WHERE id = ${movie_id}`;
    logger.warn(`Deleted movie with ID ${movie_id} from the database`);
}